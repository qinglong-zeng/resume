...
deerResume.controller("resumeCtrl", function ($scope, $http, storage) {
  ...
  if ($scope.vpass && $scope.vpass.length > 3)
    url =
      baseurl + "?a=show&domain=" + encodeURIComponent(window.location) + "&vpass=" + encodeURIComponent($scope.vpass)
  else url = baseurl + "?a=show&domain=" + encodeURIComponent(window.location)
